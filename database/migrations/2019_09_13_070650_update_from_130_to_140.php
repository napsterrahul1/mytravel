...
class UpdateFrom130To140 extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::table('bc_tours', function (Blueprint $table) {
            if (!Schema::hasColumn('bc_tours', 'include')) {
                $table->text('include')->nullable();
            }
            if (!Schema::hasColumn('bc_tours', 'exclude')) {
                $table->text('exclude')->nullable();
            }
            if (!Schema::hasColumn('bc_tours', 'itinerary')) {
                $table->text('itinerary')->nullable();
            }
        });
        Schema::table('bc_tour_translations', function (Blueprint $table) {
            if (!Schema::hasColumn('bc_tour_translations', 'include')) {
                $table->text('include')->nullable();
            }
            if (!Schema::hasColumn('bc_tour_translations', 'exclude')) {
                $table->text('exclude')->nullable();
            }
            if (!Schema::hasColumn('bc_tour_translations', 'itinerary')) {
                $table->text('itinerary')->nullable();
            }
        });
        ...
        Schema::table('bc_space_translations', function (Blueprint $table) {
            if (!Schema::hasColumn('bc_space_translations', 'extra_price')) {
