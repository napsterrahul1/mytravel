...
class SpaceSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $space_term = ['Modern Villa','Family House','Town House','Apartment','Auditorium','Bar','Cafe','Ballroom','Dance Studio','Office','Party Hall','Recording Studio','Yoga Studio','Villa','Warehouse'];
        $icon = ['flaticon-house-1','flaticon-house','flaticon-modern-house','flaticon-apartment'];

        $space_term_id = [];
        foreach ($space_term as $k=>$term){
        ...
        foreach ($IDs as $space_id){
            ...
            foreach ($space_term_id as $k => $term_space){
                if( rand(0 , count($term_ids) ) == $k) continue;
                if( rand(0 , count($term_ids) ) == $k) continue;
                if( rand(0 , count($term_ids) ) == $k) continue;
