...
namespace Database\Seeders;
    ...
    class FlightSeeder extends Seeder
    {
        public function run(){
            ...
            foreach ($argvPermission as $value){
                Permission::findOrCreate($value);
            }
            $roleAdmin = Role::findByName('administrator');
            $roleAdmin->givePermissionTo($argvPermission);
//            Seat Type
            $argvSeatType = [
                'vip'=>'Vip',
                'eco'=>'Economy',
            ...
            foreach ($argvSeatType as $item=>$value){
                $row = new SeatType(['name'=>$value,'code'=>$item]);
                $row->save();
            }
            ...
            foreach ($argvAirLineImage as $item=>$value){
                DB::table('media_files')->insertGetId(['file_name' => 'airline-'.$item, 'file_path' => $value, 'file_type' => 'image/jpeg', 'file_extension' => 'jpg']);
            }
            ...
            $a->save();
            $term_ids = [];
            foreach (['Business','First Class','Economy','Premium Economy'] as $term){
            ...
            $a->save();
            foreach (['Inflight Dining','Music','Sky Shopping','Seats & Cabin'] as $term){
                $t = new \Modules\Core\Models\Terms([
                    'name'=>$term,
                    'attr_id'=>$a->id
                ]);
            ...
            Flight::factory()->count(20)
                ->has(FlightSeat::factory()
                    ->count(3)
                    ->state(new Sequence(
                        ['seat_type'=>'vip'],
                        ['seat_type'=>'eco'],
