...
class Boat extends Bookable
{
    ...
    {
        $model_boat = parent::query()->select("bc_boats.*");
        $model_boat->where("bc_boats.status", "publish");
        if (!empty($location_id = $request->query('location_id'))) {
            $location = Location::query()->where('id', $location_id)->where("status","publish")->first();
            if(!empty($location)){
                $model_boat->join('bc_locations', function ($join) use ($location) {
                    $join->on('bc_locations.id', '=', 'bc_boats.location_id')
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( bc_boats.min_price >= ? ) AND ( bc_boats.min_price <= ? )";
        ...
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            $model_boat->join('bc_boat_term as tt', 'tt.target_id', "bc_boats.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bc_boats.review_score >= ? AND bc_boats.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
        ...
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_boat->leftJoin('bc_boat_translations', function ($join) {
                    $join->on('bc_boats.id', '=', 'bc_boat_translations.origin_id');
                });
                $model_boat->where('bc_boat_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_boat->where('bc_boats.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $model_boat->orderByRaw("POW((bc_boats.map_lng-".$lgn."),2) + POW((bc_boats.map_lat-".$lat."),2)");
        }
        ...
        $model_boat->groupBy("bc_boats.id");
