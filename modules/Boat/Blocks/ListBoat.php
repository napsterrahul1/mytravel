...
class ListBoat extends BaseBlock
{
    ...
    public function query($model){
        $model_boat = Boat::select("bc_boats.*")->with(['location','translations','hasWishList']);
        ...
        {
            $model_boat->where('bc_boats.is_featured',1);
        }
        if (!empty($model['custom_ids'])) {
            $model_boat->whereIn("bc_boats.id", $model['custom_ids']);
        }
        $model_boat->orderBy("bc_boats.".$model['order'], $model['order_by']);
