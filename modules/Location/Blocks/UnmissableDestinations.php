...
class UnmissableDestinations extends BaseBlock
{
    ...
    public function content($model = [])
    {
        ...
        $model['rows'] = $module::where('status','publish')->limit($limit)->orderBy('id', 'DESC')->with(['translations'])->get();
        $model['style'] = !empty($model['style']) ? $model['style'] :  "style_1";
        $model['modelBlock'] = $model;

        return view('Location::frontend.blocks.unmissable-destinations.'.$model['style'], $model);
    }
}
