...
class Flight extends Bookable
{
    ...
    public function getReviewList()
    {
        return $this->reviewClass::select([
            ...
            'vendor_id',
            'create_user'
        ])->where('object_id', $this->id)->where('object_model', 'Flight')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('flight_review_number_per_page', 5));
    }
    ...
    {
        $alldata = self::searchflight($request);
        $jsonall = json_decode($alldata);
    ...
    public static function searchflight(Request $request){
        //
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL,"http://api.tektravels.com/BookingEngineService_Air/AirService.svc/rest/Search/");
                curl_setopt($ch, CURLOPT_POST, 1);
                // curl_setopt($ch, CURLOPT_POSTFIELDS,"ClientId=flyproplus&UserName=flyproplus&Password=flyproplus@123&EndUserIp=123444");
                $payload = json_encode( array( "TokenId"=>$request->session()->get('key') ,
                'EndUserIp'=>$request->ip(),'AdultCount'=>1,
                'ChildCount'=>0,'InfantCount'=>0,'JourneyType'=>1,'DirectFlight'=>'false','OneStopFlight'=>'false','Sources'=>null,
                'PreferredAirlines'=>null,'Segments'=>[array('Origin'=>'BOM','Destination'=>'DEL','FlightCabinClass'=>1,'PreferredDepartureTime'=>'2022-06-28T00:00:00','PreferredArrivalTime'=>'2022-06-28T00:00:00') ]));
    ...
    public static function searchCustom(Request $request)
    {
        ...
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if (!empty($location)) {
                $model_Flight->join('bc_locations', function ($join) use ($location) {
                    $join->on('bc_locations.id', '=', 'bc_flight.location_id')->where('bc_locations._lft', '>=', $location->_lft)->where('bc_locations._rgt', '<=', $location->_rgt);
                });
            }
        }
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $model_Flight->WhereRaw($raw_sql_min_max, [
                $pri_from,
                $pri_from,
                $pri_to,
                $pri_to
        ...
        if (!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))) {
            $model_Flight->orderByRaw("POW((bc_flight.map_lng-?),2) + POW((bc_flight.map_lat-?),2)", [
                $lgn,
                $lat
            ]);
        }
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        if ($forSingle) {
            ...
            if (!empty($location_id = $this->location_id)) {
                $related = parent::query()->where('location_id', $location_id)->where("status", "publish")->take(4)->whereNotIn('id', [$this->id])->with([
                    'location',
                    'translations',
                    'hasWishList'
                ])->get();
