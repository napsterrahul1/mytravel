            ...
            @else
                ...
                $traverse = function ($locations, $prefix = '') use (&$traverse, &$list_json, &$location_name) {
                    foreach ($locations as $location) {
                        ...
                        if (request()->query('location_id') == $location->id) {
                            $location_name = $translate->city;
                        }
                        $list_json[] = [
                            'id' => $location->id,
                            'title' => $prefix . ' ' . $translate->city,
                        ];
                ...
                <div class="smart-search">
                    <input type="text" class="smart-search-location parent_text form-control"
                           {{ ( empty(setting_item("flight_location_search_style")) or setting_item("flight_location_search_style") == "normal" ) ? "" : ""  }} placeholder="{{__("Where are you going?")}}"
